    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-12">
        <div class="mb-6 sm:mb-8">
            ...
            <button 
                ...
                <span>Filters</span>
                <svg id="filter-toggle-icon" class="h-5 w-5 text-gray-500 transition-transform duration-200" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                </svg>
            </button>

            {{-- Filter section --}}
            <div id="filter-section" class="filter-section-mobile sm:flex flex-col sm:flex-row flex-wrap gap-3 sm:gap-4 lg:gap-6">
                    ...
                    <select id="nature-filter" onchange="updateFilters('nature', this.value)"
                        ...
                        @foreach ($natureTags as $tag)
                            <option value="{{ $tag->slug }}"
                                {{ request('nature') === $tag->slug ? 'selected' : '' }}>{{ $tag->name }}
                            </option>
                        @endforeach
                    </select>
                </div>
                ...
                {{-- <div class="flex flex-col w-full sm:w-auto">
                    ...
                    <select id="section-filter" onchange="updateFilters('section', this.value)"
                        ...
                        @foreach ($sectionTags as $tag)
                            <option value="{{ $tag->slug }}"
                                {{ request('section') === $tag->slug ? 'selected' : '' }}>{{ $tag->name }}
                            </option>
                        @endforeach
                    </select>
                </div> --}}
                ...
                <div class="flex flex-col w-full sm:w-auto">
                    ...
                    <select id="focus-filter" onchange="updateFilters('focus', this.value)"
                        ...
                        @foreach ($focusTags as $tag)
                            <option value="{{ $tag->slug }}"
                                {{ request('focus') === $tag->slug ? 'selected' : '' }}>{{ $tag->name }}
                            </option>
                        @endforeach
                    </select>
                </div>
                    ...
                    <a href="{{ route('projects.show', $project) }}"
                        ...
                        <div class="p-4 sm:p-6">
                            ...
                            @if ($project->supervisors->count() > 0)
                                ...
                                <div class="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-0 pt-2 sm:divide-x sm:divide-gray-400">
                                    @if ($project->section)
                                        <div class="text-xs sm:text-sm text-gray-500 sm:pr-3">{{ $project->section->name }}</div>
                                    @endif
                                    @if ($project->group)
                                        <div class="text-xs sm:text-sm text-gray-500 sm:pl-3">{{ $project->group->name }}</div>
                                    @endif
                                </div>
                            @endif
                        </div>
